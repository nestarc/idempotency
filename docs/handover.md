...
@Module({
  imports: [
    IdempotencyModule.forRoot({
      ...
      headerName: 'Idempotency-Key', // 커스텀 헤더명 (기본값 IETF draft 헤더)
...
### v0.4.0 이후
- [x] Processing TTL / in-flight lease (`processingTtl`)
- [x] 커스텀 key resolver 지원
- [x] 커스텀 fingerprint 함수 지원
- [x] 메트릭/관찰성 이벤트 (`observability.onEvent`)
- [x] Idempotency status headers (`Idempotency-Status`, `Idempotency-Replayed`)
- [x] 응답 헤더 저장/리플레이
...
- **경쟁 우위 메시지**: "IETF draft-07 호환 동작, 데코레이터 한 줄, Redis/PostgreSQL 선택"
...
Future work tracked for v0.5.0+:
